...
switch($action)
{
    ...
    case "propagate":
    {
		...
		if ($Intersections)
		{
			...
			{
	            $intersectionArr[] = $protocol . $IntIP . "/helpers/corridorDesignerHelper_v2.php";
    ...
    case "getcameralist":
    {
        ...
        if(isset($_REQUEST['ip']))
            $ip = $_REQUEST['ip'];
        $remoteURL = "$ip/helpers/insyncInterface_v2.php?action=getCameraList&u=" . base64_encode("PEC") . "&p=" . base64_encode("lenpec4321");
    ...
    case "getremoteimage":
    {
		...
		$remoteURL = "$ip/helpers/insyncInterface_v2.php?action=getImage&viewCamera=" . urlencode($cam) . "&width=160&height=120&session=false&u=" . urlencode(base64_encode("PEC")) . "&p=" . urlencode(base64_encode("lenpec4321"));
